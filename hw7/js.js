...
const snake = {
  ...
  /**
   * Переносит точку на другую сторону карты
   * @returns {{x: int, y: int}} Следующая точка в пределах карты
   */
  wrapMap(point) {
    if (point.x < 0)
      point.x = this.maxX - 1;
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        return this.wrapMap({ x: firstPoint.x, y: firstPoint.y - 1 });
      case 'right':
        return this.wrapMap({ x: firstPoint.x + 1, y: firstPoint.y });
      case 'down':
        return this.wrapMap({ x: firstPoint.x, y: firstPoint.y + 1 });
      case 'left':
        return this.wrapMap({ x: firstPoint.x - 1, y: firstPoint.y });
